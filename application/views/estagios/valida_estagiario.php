...
<div class="estagiario">
	<?php if (!isset($codigo)) {?>
		<span>Para validar seu cadastro, clique no botão abaixo. Você receberá um SMS em seu celular com um código de confirmação.</span><br><br>
		<?php if (isset($txterro)) {
			echo '<div class="erro">'.$txterro.'</div>';
	 	}
	 	
	 	if (isset($celular)) {?>
			<form action="<?php echo URL::site('estagios/estagiario'); ?>" method="post">
				<input name="num_celular" type="hidden" value="<?php echo $celular; ?>">
				<button type="submit" class="btn-blue">Gerar código</button>
			</form>
		<?php  } 
	}else { ?>
		<form action="<?php echo URL::site('estagios/verifica_codigo'); ?>" method="post">
